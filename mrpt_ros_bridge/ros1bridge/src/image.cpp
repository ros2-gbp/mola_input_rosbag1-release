...
// Ported from mrpt_ros_bridge's ros2bridge/src/image.cpp: since MRPT 3.0.0
// CImage no longer wraps cv::Mat (removed to cut dependencies), so this
// bridge copies pixels directly instead of going through OpenCV / cv_bridge,
// whose ROS2 API is in any case typed against the genuine
// sensor_msgs::msg::Image, incompatible with the vendored,
// wire-compatible sensor_msgs::Image struct used to read .bag files here.
    ...
    const mrpt::img::CImage& img, const std_msgs::Header& msg_header)
{
  ...
  msg.encoding =
      isColor ? sensor_msgs::image_encodings::RGB8 : sensor_msgs::image_encodings::MONO8;
  msg.is_bigendian = 0;
